...
DEFAULT_CHRONOS_CONFIG = {
    ...
    "fine_tune_mode":   "lora",
    "fine_tune_batch_size": 256,   # only used when fine_tune_steps > 0

    # ── Compute ──────────────────────────────────────────────────────────────
    "device":  "cuda" if torch.cuda.is_available() else "cpu",  # "gpu" or "cpu"
    "batch_size":       256,     # reduce to 8-16 if GPU OOM on large model
...
class M5ExplorationSuite:
    ...
    def __init__(
        ...
        # cache to avoid rebuilding identical TimeSeriesDataFrames across experiments
        self._cached_tsdf = {} 
        self._cached_future = {}
    ...
    def _build_tsdfs(
        ...
        """
        Check caching: mismatched columns/data may cause bugs[cite: 1, 2]. 
        Add data hashes or timestamps to cache keys to ensure data and column sets stay synchronized[cite: 1].
        """

        # ---- Build cache keys ----
        train_cols = ["id", "date", "sales_quantity"] + known_cov_cols + past_cov_cols
        train_cols = tuple(sorted([c for c in train_cols if c in hist_df.columns]))

        train_key = (train_cols, use_static)
        ...
        if known_cov_cols:
            if future_key in self._cached_future:
                ...
                self._cached_future[future_key] = ag_future
        else:
            ag_future = None
    ...
    def _chronos_hp(self, cfg: dict) -> dict:
        ...
        Key mapping:
          fine_tune_steps > 0  → enables gradient updates on M5 training data.
          fine_tune_mode       → "lora" or "full" (passed as optimization_strategy).
          context_length       → encoder token budget.
          known_cov_cols != [] → only effective when fine_tune_steps > 0.
        """
        hp: dict[str, Any] = {
            ...
            "device":         cfg.get("device",         DEFAULT_CHRONOS_CONFIG["device"]), # commented out to let AutoGluon decide based on availability
        ...
        if fine_tune_steps > 0:
            ...
            hp["fine_tune_batch_size"] = cfg.get("fine_tune_batch_size", 128)
